...
public enum Format {
    ...
    private static final Format[] formatTable = new Format[256];

    static {
        // Preparing a look up table for converting byte values into Format types
        for (int b = 0; b <= 0xFF; ++b) {
            Format mf = toFormat((byte) b);
            formatTable[b] = mf;
        }
    }
    ...
    private Format(ValueType valueType) {
     ...
     * @return
     */
    public static Format valueOf(final byte b) {
     ...
     * @return
     */
    static Format toFormat(final byte b) {
    ...
    public ValueType getValueType()
            throws Exception {
        if (this == NEVER_USED) {
            throw new Exception("Cannot convert NEVER_USED to ValueType");
        }
        return valueType;
    }
}
